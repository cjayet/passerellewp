<h2 id="bloc_image">PUSH image </h2>
<form>

    <div class="form-group">
        <label for="new_title">URL image</label>
        <input type="text" class="form-control" data-id="url_image" placeholder="" value="http://localhost/wordpress/wp-content/uploads/2016/11/corporate-2-westport-house-img-1-300x200.jpg" />
    </div>

    <div class="form-group">
        <label for="new_title">Alt à lui attribuer</label>
        <input type="text" class="form-control" data-id="update_image_alt" placeholder="Image alt" value="" />
    </div>

    <button type="button" data-id="action" class="btn btn-default push_cms" value="set_img_alt">Envoyer DEPRECATED</button>
</form>

<br /><hr /><br />